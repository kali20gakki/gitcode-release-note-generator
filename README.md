...
提供 GitCode 仓库链接、roadmap、版本时间范围和 token，Agent 会自动拉取该时间范围内的 PR、Issue 和 roadmap 数据，按标准模板生成结构化的 Release Note Markdown 文件。
...
Token 需要至少拥有 `pull_requests` 和 `issues` 读取权限。
...
- 读取 **Roadmap Issue** 内容提取版本亮点
- 按标签和标题关键词自动分类：新增特性、变更说明、修复缺陷、已知问题
...
1. **版本概述** — 版本定位、面向场景、核心亮点（从 roadmap 自动提取）
2. **配套关系** — 软硬件版本要求（需人工补充）
3. **新增特性** — 时间范围内的新功能 PR/Issue
4. **变更说明** — 接口调整、行为变化等（含不兼容变更标注）
5. **修复缺陷** — Bug 修复列表
6. **已知问题** — 未修复的遗留问题（需人工补充）
7. **致谢** — 贡献者列表
...
| 分类 | 触发条件 |
|------|----------|
| 新增特性 | 标签含 `feature`/`enhancement`/`需求`，或标题含 `新增`/`支持`/`添加` |
| 变更说明 | 标签含 `change`/`变更`/`调整`/`修改`，或其他未分类 PR |
...
Skill 生成的是 **初稿**，**必须人工审查并补充**：

- **版本概述**：补充准确的场景描述和亮点总结
- **配套关系**：填写实际的软硬件版本要求
- **特性描述**：为每条特性补充用户视角的描述（脚本只填了标题）
